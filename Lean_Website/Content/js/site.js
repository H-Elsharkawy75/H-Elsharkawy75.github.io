...
function readURL(input) {
    if (input.files && input.files[0]) {
        ...
        $(input).closest('.img-collection').find('input[type=hidden][name=' + $(input).attr('name') + ']').val('');
        $(input).closest('.img-collection').find('img').attr('src', '/assets/placeholder.png');
    }
}
    ...
    if (input.files && input.files[0]) {
        for (var i = 0; i < input.files.length; i++) {
            ...
            reader.onload = function (e) {
                $target.append(`
                    <div class='img-item'>
                        <img width='100' height='100' class='m-1' src='${e.target.result}' onerror='this.src="/assets/pdf.png"' />
                        <i class="fa fa-times-circle remove" onclick='removeImg(this)'></i>
                    </div>`
                );
            };
...
function OrderTableNames(tblId, prefix = '') {
    ...
    let hash = tblId.includes('#') ? '' : '#';
    $(`${hash + tblId} tbody tr[class!=exclude]`).each((i, tr) => {
        $(tr).find('input[class!=exclude],select[class!=exclude],textarea').each((x, input) => {
...
function ResetSearch(element) {
    ...
    //$(element).closest('form').find('select').val('');
    $(element).closest('form').find('option:first-child').prop('selected', 'selected');
    $(element).closest('form').find('input[type=checkbox]').prop('checked', false);
...
// MODAL EDIT
function LoadPartialInModal(url, title, largeModal = false, afterLoad = null) {
...
function LoadPartialInSpecialModal(url, afterLoad = null) {
    $('#SpecialModal').find('.modal-body').load(url, () => {
        $('#SpecialModal').modal('show');
        if (afterLoad) afterLoad();
    });
}

function LoadPartialInImageModal(url, afterLoad = null) {
    $('#ImageModal').find('.modal-body').load(url, () => {
...
var getPagination = function () {
    ...
    let _serialized = '';
    let formserialize = $('.pagination-filter-form').serialize();
    let length = formserialize.split('&').length < 20 ? formserialize.split('&').length : 20;
    for (let i = 0; i < length; i++) {
        _serialized += $('.pagination-filter-form').serialize().split('&')[i] + (i < (length - 1) ? '&' : '');
    }
...
var paginationFilterFormSubmit = function () {
    ...
    let length = formserialize.split('&').length < 20 ? formserialize.split('&').length : 20;
    for (let i = 0; i < length; i++) {
        _serialized += $form.serialize().split('&')[i] + (i < (length - 1) ? '&' : '');
    }
